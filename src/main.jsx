...
function DockMap({liveDocks,expanded,onExpand,onClose}) {
 ...
 return <section className={`panel dock-panel ${expanded?'dock-modal':''}`}>
  ...
  <div className="dock-grid">{docks.map(d=>{const active=liveDocks[d.id];const reserved=active?.status==='Chamado para doca';return <article key={d.id} className={`dock-card ${d.blocked?'blocked':''} ${active?'occupied':''} ${reserved?'reserved':''}`}><div className="dock-number"><Truck size={17}/><strong>{d.id}</strong></div><div className="dock-bases">{d.bases.map(b=><span key={b}>{b}</span>)}</div>{active?<div className="dock-live"><b>{active.plate}</b><span>Rota {active.route||'não informada'}</span></div>:null}<small>{d.blocked?'INTERDITADA':reserved?'CONVOCADO':active?'OCUPADA':'DISPONÍVEL'}</small></article>})}</div>
...
function Dashboard({onScan}) {
 ...
 const liveDocks=useMemo(()=>Object.fromEntries(activeDrivers.filter(d=>(d.status==='Endocado'&&d.dockId)||(d.status==='Chamado para doca'&&d.assignedDock)).map(d=>[d.dockId||d.assignedDock,d])),[activeDrivers]);
 ...
 const saveEvent=async(plate,fields,event)=>{if(!firebaseReady||!db)return;await setDoc(doc(db,'presencas',plate),{...fields,updatedAt:serverTimestamp()},{merge:true});await addDoc(collection(db,'movimentacoes'),{plate,event,...fields,createdAt:serverTimestamp()})};
 const callToDock=d=>{const dockId=assignments[d.plate]||availableDocks[0]?.id;if(dockId)saveEvent(d.plate,{status:'Chamado para doca',assignedDock:dockId,location:`Chamado para Doca ${dockId}`,calledAt:serverTimestamp()},'CHAMADO_PARA_DOCA')};
 const finishVehicle=d=>saveEvent(d.plate,{status:'Veículo liberado',location:'Carga/descarga finalizada',dockId:null,assignedDock:null,releasedAt:serverTimestamp()},'VEICULO_LIBERADO');
 const setDelayReason=(d,reason)=>saveEvent(d.plate,{delayReason:reason},'MOTIVO_ATRASO');
 return <>
  ...
  <section className="grid-main"><div className="panel live"><div className="panel-head"><div><h2>Fila operacional</h2><p>Ordem de chegada, convocação e liberação</p></div><div className="search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar placa ou motorista"/></div></div><div className="table-wrap"><table className="queue-table"><thead><tr><th>FILA</th><th>MOTORISTA</th><th>LOCALIZAÇÃO</th><th>ESPERA</th><th>ROTA</th><th>STATUS</th><th>AÇÃO OPERACIONAL</th></tr></thead><tbody>{filtered.length?filtered.map((d,index)=>{const mins=minutesWaiting(d);return <tr key={d.plate}><td><b>#{index+1}</b></td><td><b>{d.plate}</b><span>{d.name||'Cadastro pendente'}</span>{d.registered===false?<em className="new-plate">PLACA NOVA</em>:null}</td><td><b className="place"><MapPin size={14}/>{d.place||d.location}</b><span>{d.carrier||''}</span></td><td><span className={`time-chip ${timeTone(mins)}`}>{mins} min</span>{mins>60?<select className="reason-select" value={d.delayReason||''} onChange={e=>setDelayReason(d,e.target.value)}><option value="">Motivo do atraso</option><option>Doca ocupada</option><option>Aguardando documentação</option><option>Motorista não localizado</option><option>Aguardando liberação operacional</option><option>Problema no veículo</option><option>Divergência de carga</option><option>Outro</option></select>:null}</td><td>{d.route||'—'}</td><td><span className={`pill ${d.status?.includes('Aguardando')?'wait':d.status==='Chamado para doca'?'called':'work'}`}>{d.status}</span></td><td>{d.status?.includes('Aguardando')?<div className="row-action"><select value={assignments[d.plate]||''} onChange={e=>setAssignments({...assignments,[d.plate]:e.target.value})}><option value="">Selecionar doca</option>{availableDocks.map(x=><option key={x.id}>{x.id}</option>)}</select><button onClick={()=>callToDock(d)}>Chamar</button></div>:d.status==='Endocado'?<button className="finish-button" onClick={()=>finishVehicle(d)}>Finalizar carga/descarga</button>:<span className="action-note">Aguardando endocamento</span>}</td></tr>}):<tr><td colSpan="7" className="empty-row">Nenhum motorista no pátio</td></tr>}</tbody></table></div></div>
  <aside className="panel summary"><div className="panel-head"><div><h2>Ocupação</h2><p>Capacidade operacional</p></div></div><div className="capacity"><strong>{activeDrivers.length}</strong><span>veículos no CDC</span><progress max="30" value={activeDrivers.length}/><small>{Math.round(activeDrivers.length/30*100)}% da capacidade de 30 veículos</small></div><div className="legend"><p><i className="dot parking"/>Aguardando <b>{waiting}</b></p><p><i className="dot dock"/>Endocados <b>{docked}</b></p><p><i className="dot moving"/>Chamados <b>{activeDrivers.filter(d=>d.status==='Chamado para doca').length}</b></p></div>{newPlates>0?<div className="alert red-alert"><AlertTriangle size={18}/><div><b>{newPlates} placa(s) aguardando cadastro</b><span>Complete os dados dos novos motoristas.</span></div></div>:null}<div className="alert"><AlertTriangle size={18}/><div><b>Doca 58 interditada</b><span>SBD e AGFI sem operação nesta posição.</span></div></div></aside></section>
...
function Scan({onBack}) { const [plate,setPlate]=useState(''); const [route,setRoute]=useState(''); const [done,setDone]=useState(false); const [saving,setSaving]=useState(false); const [error,setError]=useState(''); const params=new URLSearchParams(locationSearch()); const location=params.get('local')||'Próximo da Doca 52'; const dockId=params.get('doca'); const isArrival=location==='Portaria'; const eventName=isArrival?'Chegada no CDC':dockId?`Endocado — Doca ${dockId}`:location; const confirm=async()=>{if(!firebaseReady||!db){setDone(true);return}setSaving(true);setError('');try{const normalized=plate.toUpperCase();const driverQuery=query(collection(db,'motoristas'),where('plate','==',normalized),limit(1));const registered=!(await getDocs(driverQuery)).empty;const status=dockId?'Endocado':'Aguardando';const stageTime=isArrival?{arrivalAt:serverTimestamp()}:dockId?{dockedAt:serverTimestamp()}:{};const payload={plate:normalized,route:route.toUpperCase(),location:eventName,status,dockId:dockId||null,registered,...stageTime,updatedAt:serverTimestamp()};await setDoc(doc(db,'presencas',normalized),payload,{merge:true});await addDoc(collection(db,'movimentacoes'),{...payload,event:isArrival?'CHEGADA_NO_CDC':dockId?'ENDOCADO':'LOCALIZACAO_PATIO',createdAt:serverTimestamp()});if(!registered)await setDoc(doc(db,'alertas',`placa-${normalized}`),{type:'PLACA_NOVA',plate:normalized,route:route.toUpperCase(),read:false,createdAt:serverTimestamp()},{merge:true});setDone(true)}catch(e){setError('Não foi possível registrar. Tente novamente.')}finally{setSaving(false)}}; return <div className="scan-page"><div className="scan-card"><div className="scan-brand"><span><QrCode/></span><b>Controle de Pátio</b></div>{done?<div className="success"><div className="success-icon"><CircleCheck size={42}/></div><p className="eyebrow">LOCALIZAÇÃO CONFIRMADA</p><h1>Registro realizado!</h1><p>Sua localização foi enviada ao controle operacional.</p><div className="receipt"><span>Placa<b>{plate.toUpperCase()}</b></span><span>Rota<b>{route.toUpperCase()}</b></span><span>Registro<b>{eventName}</b></span><span>Horário<b>{new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</b></span></div><button className="primary full" onClick={onBack}>Concluir</button></div>:<><p className="eyebrow">CONFIRMAÇÃO DE LOCALIZAÇÃO</p><h1>Você está em<br/><em>{eventName}</em></h1><p className="scan-help">Informe somente a placa e a rota para confirmar sua localização.</p><label>Placa do veículo</label><input className="plate-input" maxLength="7" value={plate} onChange={e=>setPlate(e.target.value.replace(/[^a-zA-Z0-9]/g,''))} placeholder="ABC1D23"/><label>Rota</label><input className="route-input" value={route} onChange={e=>setRoute(e.target.value)} placeholder="Informe a rota"/>{error?<p className="form-error">{error}</p>:null}<button disabled={plate.length<7||!route.trim()||saving} className="primary full" onClick={confirm}>{saving?'Registrando...':'Confirmar localização'} <ChevronRight size={18}/></button><small className="safe">Se a placa for nova, a operação será avisada para completar o cadastro.</small></>}</div></div> }
